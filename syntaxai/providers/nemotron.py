"""Nemotron provider for SyntaxAI."""
...
class NemotronProvider(LLMProvider):
    def __init__(self, api_key: str, model: str = "nemotron-mini",
                 base_url: str = ""):
        super().__init__(api_key, model, base_url or "https://apiquery.nemotron.io")
    ...
    def _fallback_response(self, prompt: str, context: str, error: str = "") -> LLMResponse:
        return LLMResponse(
            content=f"Error connecting to Nemotron API: {error}\n\nPrompt: {prompt[:100]}",
            tool_calls=[],
            usage={},
            model=self.model,
            finish_reason="error"
...
class NemotronProProvider(NemotronProvider):
    def __init__(self, api_key: str):
        super().__init__(api_key, model="nemotron-pro")


class NemotronFlashProvider(NemotronProvider):
    def __init__(self, api_key: str):
        super().__init__(api_key, model="nemotron Flash")